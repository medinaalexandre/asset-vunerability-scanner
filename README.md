...
## Observability and Monitoring (Datadog Integration)

The application is fully integrated with **Datadog** to ensure comprehensive observability across the entire stack,
from container performance to application latency and structured logging.

### Infrastructure Monitoring (Containers)

The Datadog Agent, deployed as a container, collect host and container-level metrics.
...
Laravel's logging system is configured to send structured JSON logs (via Monolog/TCP) to Datadog.
...
and requests.
The interactive Swagger/OpenAPI documentation is available locally at `/docs/api`.
